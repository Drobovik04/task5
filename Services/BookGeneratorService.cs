...
namespace task5.Services
{
    public class BookGeneratorService
    {
        ...
        {
            var faker = new Faker<Book>(settings.Language).UseSeed(settings.Seed + batch);
            var books = new List<Book>();
            faker.RuleFor(o => o.Index, s => batch * batchSize + startIndex++)
                .RuleFor(o => o.ISBN, s => s.Random.Replace("###-#-###-#####-#"))
                .RuleFor(o => o.Title, s => s.Lorem.Sentence())
                .RuleFor(o => o.Author, s => s.Name.FullName())
                .RuleFor(o => o.Publisher, s => s.Company.CompanyName())
        ...
        private List<Review> GenerateReviews(Faker faker, int reviewsCount)
        {
            ...
            {
                reviews.Add(new Review() { Reviewer = faker.Name.FullName(), ReviewText = faker.Lorem.Paragraph()});
            }
